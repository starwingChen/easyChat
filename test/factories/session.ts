...
interface CreateSessionOptions extends Partial<Omit<ChatSession, 'selectedModels' | 'messages' | 'activeBotIds'>> {
  bots?: SessionBot[];
  activeBotIds?: string[];
...
export function createSession(options: CreateSessionOptions = {}): ChatSession {
  ...
  const selectedModels =
    options.selectedModels ?? Object.fromEntries(bots.map((bot) => [bot.id, bot.defaultModel]));
