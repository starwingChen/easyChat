...
export function createBotDefinition(overrides: Partial<BotDefinition> = {}): BotDefinition {
  const models = overrides.models && overrides.models.length > 0 ? overrides.models : defaultModels;
  const defaultModel =
    overrides.defaultModel ?? models.find((model) => model.isDefault)?.id ?? models[0].id;
