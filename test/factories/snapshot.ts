...
interface CreateSnapshotOptions
  extends Partial<Omit<SessionSnapshot, 'sourceSessionId' | 'activeBotIds' | 'selectedModels' | 'messages'>> {
...
export function createSnapshot(options: CreateSnapshotOptions = {}): SessionSnapshot {
  const session = options.session;
  const messages = [...(options.messages ?? session?.messages ?? [])];
  const activeBotIds = [...(options.activeBotIds ?? session?.activeBotIds ?? [])];
  const selectedModels = options.selectedModels ?? session?.selectedModels ?? {};
  const title = options.title ?? session?.title ?? 'History Session';
  const layout = options.layout ?? session?.layout ?? '2v';
  const sourceSessionId = options.sourceSessionId ?? session?.id ?? 'session-active';
