...
export function getSnapshotBrowseableBotIds(
  snapshot: Pick<SessionSnapshot, "messages" | "activeBotIds">,
): string[] {
...
export function createSnapshotFromSession(
  ...
  const repliedBotIds = getSnapshotRepliedBotIds({
  ...
  const titleSeed =
    snapshotMessages
      ...
      ?.content.trim() || session.title;
  const title =
    titleSeed.length > 32 ? `${titleSeed.slice(0, 32)}...` : titleSeed;
  ...
  return {
    ...
    title,
    layout:
      repliedBotIds.length > 0
        ? deriveSnapshotLayout(repliedBotIds.length)
        : session.layout,
    activeBotIds,
