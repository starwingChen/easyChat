...
describe('appReducer', () => {
  it('hydrates locale/view/history/sidebar and re-computes active bots for the resolved layout', () => {
    ...
    expect(next.currentView).toEqual({
      mode: 'active',
      sessionId: 'session-active',
    });
    expect(next.historySnapshots.map((snapshot) => snapshot.id)).toEqual([
      'hist-1',
    ]);
  ...
  it('sets layout and ensures there are enough unique active bots for that layout without trimming', () => {
    ...
    const next = appReducer(state, {
      ...
      payload: {
        layout: '3',
        allBotIds: ['gemini', 'chatgpt', 'perplexity', 'deepseek-api'],
      },
    });
    ...
    expect(next2.activeSession.activeBotIds).toEqual([
      'gemini',
      'chatgpt',
      'perplexity',
    ]);
  ...
  it('replaces an existing active message and updates updatedAt', () => {
    ...
    expect(next.activeSession.messages[1]).toEqual(
      expect.objectContaining({ id: 'assistant-1', content: 'new' })
    );
  });
