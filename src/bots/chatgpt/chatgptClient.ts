...
type JsonFetcher = (request: string, options?: FetchOptions) => Promise<unknown>;
...
function buildHeaders(accessToken: string, deviceId: string, language: string): HeadersInit {
  return {
...
export function createChatGPTClient(options: ChatGPTClientOptions = {}): ChatGPTClient {
  const fetchJson = options.fetchJson ?? ((request, init) => ofetch(request, init));
  ...
  async function getChatRequirements(accessToken: string): Promise<ChatGPTRequirements> {
    const requirementsToken = await sentinel.createRequirementsToken();
    const requirements = parseChatGPTRequirements(
      await fetchJson(`${CHATGPT_BASE_URL}/backend-api/sentinel/chat-requirements`, {
        method: 'POST',
        body: {
          p: requirementsToken,
        },
  ...
  async function sendConversationMessage(
    ...
    const response = await fetchNative(`${CHATGPT_BASE_URL}/backend-api/conversation`, {
      method: 'POST',
      signal: input.signal,
      headers: {
        ...
        'Openai-Sentinel-Chat-Requirements-Token': input.chatRequirementsToken,
        ...(input.proofToken
          ? {
