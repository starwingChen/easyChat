...
export function parseGeminiGenerateResponse(responseText: string): GeminiGenerateResult {
  ...
  for (const payload of payloads) {
    ...
    const ids = Array.isArray(payload[1]) ? payload[1].filter((item): item is string => typeof item === 'string') : [];
    const candidateBlocks = Array.isArray(payload[4]) ? payload[4] : [];
    const firstBlock = Array.isArray(candidateBlocks[0]) ? candidateBlocks[0] : null;
    ...
    if (blockText) {
      text = blockText;
    } else if (!text && Array.isArray(partialText) && typeof partialText[0] === 'string') {
      text = partialText[0];
    }
