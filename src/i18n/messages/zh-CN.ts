const zhCNMessages = {
  ...
  'sidebar.feedback.open': '打开github页面',
  'sidebar.settings.open': '打开配置',
  'sidebar.settings.title': '配置',
  'sidebar.settings.close': '关闭配置',
  'sidebar.settings.language': '切换中英文',
  'sidebar.settings.feedback': '反馈',
  'sidebar.settings.feedbackSubtitle': '觉得好用可以点个 star 支持一下',
  ...
  'config.apiKey.show': '显示 API Key',
  'config.apiKey.hide': '隐藏 API Key',
  'config.modelName': '调用模型名称',
  'config.modelPicker.add': '添加模型',
  'config.modelPicker.remove': '删除模型',
  'config.modelPicker.empty': '还没有已保存的模型',
  'config.unset': '未配置',
  'config.cancel': '取消',
  'config.save': '保存',
