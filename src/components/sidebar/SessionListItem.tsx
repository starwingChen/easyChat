...
export function SessionListItem({
  ...
  return (
    <div className="group relative">
      ...
      {onDeleteRequest ? (
        <button
          aria-label={t ? t('sidebar.deleteHistory') : 'Delete history session'}
      ...
      {isDeleteConfirming ? (
        <div className="absolute right-0 top-[calc(100%+0.35rem)] z-20 w-42 rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_12px_30px_rgba(15,23,42,0.12)]">
          <div className="text-xs font-medium text-slate-600">
            {t ? t('sidebar.deleteConfirm') : 'Delete this snapshot?'}
          </div>
          <div className="mt-3 flex justify-end gap-2">
            <button
              ...
              type="button"
            >
              {t ? t('sidebar.cancelDelete') : 'Cancel'}
            </button>
            <button
              ...
              type="button"
            >
              {t ? t('sidebar.confirmDelete') : 'Delete'}
            </button>
          </div>
        </div>
      ) : null}
