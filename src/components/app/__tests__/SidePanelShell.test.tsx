...
describe("SidePanelShell", () => {
  it("shows a collapsed settings trigger with divider and opens the dialog", async () => {
    ...
    expect(
      screen.getByTestId("collapsed-sidebar-settings-divider"),
    ).toBeInTheDocument();
  ...
  it("renders only the latest 10 history buttons in collapsed mode", () => {
    ...
    expect(
      screen.getByRole("button", { name: "History 1" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "History 10" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "History 11" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "History 12" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders dividers after expand, new session, and current session", () => {
    ...
    expect(
      screen.getAllByTestId("collapsed-sidebar-section-divider"),
    ).toHaveLength(3);
  });
  ...
  it("switches back to the active session from the collapsed rail", async () => {
    ...
    expect(selectView).toHaveBeenCalledWith({
      mode: "active",
  ...
  it("marks only the selected history icon as current when history view is selected", () => {
    buildUseAppState({
      currentView: { mode: "history", sessionId: "hist-2" },
      historyCount: 3,
    });
